...
class ArcSeekBar  @JvmOverloads constructor(
    ...
    var progress: Int = a.useOrDefault(0) { getInteger(R.styleable.ArcSeekBar_progressArc, it) }
        set(progress) {
            field = bound(0, progress, maxProgress)
            onProgressChangedListener?.invoke(progress, false)
    ...
    var progressBackgroundWidth: Float = a.useOrDefault(2F) { getDimension(R.styleable.ArcSeekBar_progressBackgroundWidthArc, it) }
        set(mArcWidth) {
            field = mArcWidth
    ...
    private var progressBackgroundPaint: Paint = makeProgressPaint(
            color = a.useOrDefault(resources.getColor(android.R.color.darker_gray)) { getColor(R.styleable.ArcSeekBar_progressBackgroundColorArc, it) },
        ...
        if (mEnabled) {
            when (event.action) {
                MotionEvent.ACTION_DOWN -> {
                    onStartTrackingTouch?.invoke(progress, true)
                    updateOnTouch(event)
                }
                MotionEvent.ACTION_MOVE -> {
                    onProgressChangedListener?.invoke(progress, true)
                    updateOnTouch(event)
                }
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    onStopTrackingTouch?.invoke(progress, true)
                    isPressed = false
                }
            }
        }
