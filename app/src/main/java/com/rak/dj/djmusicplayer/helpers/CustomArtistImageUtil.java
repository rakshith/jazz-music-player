...
public class CustomArtistImageUtil {
    ...
    public void setCustomArtistImage(final Artist artist, Uri uri) {
        Glide.with(JazzMusicPlayerApp.getInstance())
                .load(uri)
                .asBitmap()
                ...
                .into(new SimpleTarget<Bitmap>() {
                    ...
                    public void onLoadFailed(Exception e, Drawable errorDrawable) {
                        ...
                        e.printStackTrace();
                        Toast.makeText(JazzMusicPlayerApp.getInstance(), e.toString(), Toast.LENGTH_LONG).show();
                    }
                    ...
                    public void onResourceReady(final Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                        new AsyncTask<Void, Void, Void>() {
                            ...
                            protected Void doInBackground(Void... params) {
                                File dir = new File(JazzMusicPlayerApp.getInstance().getFilesDir(), FOLDER_NAME);
                                if (!dir.exists()) {
                                ...
                                if (succesful) {
                                    ...
                                    ArtistSignatureUtil.getInstance(JazzMusicPlayerApp.getInstance()).updateArtistSignature(artist.getName());
                                    JazzMusicPlayerApp.getInstance().getContentResolver().notifyChange(Uri.parse("content://media"), null); // trigger media store changed to force artist image reload
                                }
                                return null;
                            }
                        }.execute();
